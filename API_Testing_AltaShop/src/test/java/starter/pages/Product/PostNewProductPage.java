...
public class PostNewProductPage {
    ...
    public void sendRequestBodyPostCreateNewProduct() {
        ...
        postcreate.put("categories",new JSONArray().put(12126));
    ...
    public void validateResponseBodyPostCreateNewProduct() {
        ...
        restAssuredThat(response -> response.body("data.Categories[0].ID", equalTo(12126)));
        restAssuredThat(response -> response.body("data.Categories[0].Name", equalTo("Accessories")));
        restAssuredThat(response -> response.body("data.Categories[0].Description", equalTo("to enhance appearance")));
    }
