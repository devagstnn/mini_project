...
public class GetOrderByIdPage {
    ...
    public String apiEndpointGetOrderById() {
        return PostNewOrderPage.endpoint + "/10438";
    }
    ...
    public void validateResponseBodyGetOrderById() {
        restAssuredThat(response -> response.body("data.ID",equalTo(10438)));

        restAssuredThat(response -> response.body("data.User.ID",equalTo(18559)));
        ...
        restAssuredThat(response -> response.body("data.Product.ID",equalTo(13425)));
